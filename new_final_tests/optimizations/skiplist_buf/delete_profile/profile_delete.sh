...
# Note: this script should be run as `./profile_delete.sh 1gb` to test executing 100K delete queries on a database with 1gb of data
...
data_size_string=$1
workload_filename="100K_delete.txt"
...
if [ $PROGRAM_EXIT_CODE -ne 0 ]; then
    echo "Program exited with error during ${data_size_string} delete workload."
else
    echo "Profiling complete for ${data_size_string} delete workload."
fi