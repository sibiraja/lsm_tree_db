...
data_size_string=$1
expected_entries=$2
workload_filename="../${data_size_string}_put_200K_get.txt"
...
# Navigate to the directory of the database executables (currently in a subdir inside optimizations subdir)
cd ../../../../
...
{ time ./database_both "$expected_entries" < "${script_dir}/${workload_filename}" ; } >> "$time_database_output" 2>&1
PROGRAM_EXIT_CODE=$?
...
if [ $PROGRAM_EXIT_CODE -ne 0 ]; then
    echo "Program exited with error during varying fence ptr AND size ratio database variant for ${data_size_string} put workload."
else
    echo "Profiling complete for varying fence ptr AND size ratio database variant for ${data_size_string} put workload."
fi