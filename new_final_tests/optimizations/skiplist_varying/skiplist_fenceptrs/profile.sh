...
{ time ./database_fence "$expected_entries" < "${script_dir}/${workload_filename}" ; } >> "$time_database_output" 2>&1
PROGRAM_EXIT_CODE=$?
...
if [ $PROGRAM_EXIT_CODE -ne 0 ]; then
    echo "Program exited with error during varying fence ptr database variant for ${data_size_string} put workload."
else
    echo "Profiling complete for  varying fence ptr database variant for ${data_size_string} put workload."
fi