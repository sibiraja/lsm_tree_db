...
WORKLOAD_PATH="final_tests/data_size/100mb_put.txt"

# Start iostat to monitor IO statistics and log them to a file. Run in the background.
iostat -dx 2 > final_tests/data_size/100mb_iostat_log.txt &
...
valgrind --tool=cachegrind ./database < $WORKLOAD_PATH > final_tests/data_size/100mb_cachegrind_output.txt 2>&1
