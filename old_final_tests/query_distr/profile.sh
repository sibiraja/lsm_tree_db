...
for SKEW in "${SKEWNESS_LEVELS[@]}"; do
    ...
    # Define the input file based on current skewness level
    INPUT_FILE="${script_dir}/skewed_${SKEW}_put_get.txt"
    # Define output files for cachegrind and iostat
    CACHEGRIND_OUT="${script_dir}/cachegrind_${SKEW}_output.txt"
    IOSTAT_OUT="${script_dir}/iostat_${SKEW}_output.txt"
    ...
    if [ $PROGRAM_EXIT_CODE -ne 0 ]; then
        echo "Program exited with error during skew ${SKEW}% workload."
    else
        echo "Profiling complete for skew ${SKEW}% workload."
    fi
